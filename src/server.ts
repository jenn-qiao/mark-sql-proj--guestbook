...
app.get("/signatures/:id", async (req, res) => {
  ...
  const signature = await client.query("SELECT * FROM signatures WHERE id = $1", [id]);   
...
app.post("/signatures", async (req, res) => {
  ...
  if (typeof name === "string") {
    const createdSignature = await client.query("INSERT INTO signatures (signature,message) VALUES ($1,$2) RETURNING *", [name, message]); 
...
app.put("/signatures/:id", async (req, res) => {
  ...
  if (typeof name === "string") {
    ...
    const result: any = await client.query("UPDATE signatures SET signature = $1, message = $2 where id = $3", [name,message, id]); 
